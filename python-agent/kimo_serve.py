#!/usr/bin/env python3
"""خادم المحرك — يكشف محرك كيمو عبر HTTP ليُستدعى من أي واجهة (ويب أو إكسبو).

المسار الوحيد لتشغيل محرك بايثون من تطبيق إكسبو (الذي لا يشغّل بايثون داخله)
هو جسر HTTP: يعمل هذا الخادم على الجهاز، ويتصل به تطبيق الإكسبو عبر الشبكة.

نقاط النهاية:
  GET  /api/providers        قائمة المزوّدات
  GET  /api/settings         إعدادات المحرك الحالية
  POST /api/settings         حفظ الإعدادات
  POST /api/session          إنشاء جلسة محادثة
  POST /api/chat             إرسال رسالة وانتظار الرد النهائي
  GET  /                     شاشة محادثة ويب بسيطة (للتجربة من المتصفح)
...
class EngineHub:
    """يحمل نسخة واحدة من المحرك ويخدم الجلسات عبر HTTP."""
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/settings":
            ...
            save_settings(s)
            # إعادة بناء المحرك ليعكس الإعدادات الجديدة
            global HUB
            HUB = None
...
<body><div class="wrap">
<header>محادثة كيمو — المحرك البايثوني</header>
<div id="log"></div>
...
def main() -> None:
    ...
    print(f"خادم محرك كيمو يعمل على:  http://localhost:{PORT}")
    print("لتجربته من تطبيق إكسبو، استخدم عنوان IP لهذا الجهاز على الشبكة نفسها.")
