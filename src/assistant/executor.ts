...
import { buildSystemPrompt, buildMinimalPrompt, getAgentFunctions, getAllAgentFunctions } from './prompts'
...
async function runLoop(
  ...
  try {
    ...
    const readIntentRequiresEvidence = Boolean(
      lastUserMsg
      && /(?:اعرض|أظهر|اظهر|اقرأ|استكشف|ابحث|اعثر|كم|عدد|إجمالي|ملخص|جدول|شجرة|المؤشرات|التدفقات|الأقساط|المشروع|الوقت المحلي|تاريخ اليوم|البيانات المحلية|التنبيهات|التدقيق|من غيّر|وريني|ورني|هات|طلع|جيب|اطلع)/.test(lastUserText)
      && !/(?:أنشئ|انشئ|أضف|اضف|ضيف|عدّل|عدل|حدّث|حدث|احذف|حذف|مسح|شيل|سجّل|سجل\s+(?:دفعة|دفع|مبلغ|قسط|إيصال|تحويل)|استورد|استيراد|ذكرني|تذكير|سوي|اعمل|احجز|قيد|صلح)/.test(lastUserText),
    )
    ...
    if (lastUserMsg) {
      ...
      const continuationMessage = goal.startsWith(ASK_PREFIX) || goal.startsWith(APPROVE_PREFIX) || goal.startsWith(REFUSE_PREFIX)
